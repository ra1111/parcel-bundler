...
module.exports = {
  ...
  CallExpression(path, asset) {
    ...
    if (referencesImport(path, 'fs', 'readFileSync')) {
      ...
      let argNodes = path.get('arguments');

      let [filename, args] = [null, []];

      let logger = Logger.forAsset(asset);

      for (let i = 0; i < argNodes.length; i++) {
        let arg = argNodes[i];

        let evaluated = evaluate(arg, vars);

        let type = i === 0 ? 'filename' : 'options';

        if (evaluated === nodeNotEvaluated) {
          let {column, line} = arg.node.loc.start;
          ...
          logger.warn(
            `${file}: Cannot statically evaluate fs ${type} argument`
          );
...
function evaluate(path, vars) {
  ...
  return res.confident ? res.value : nodeNotEvaluated;
}
